...
from .utils import Stack, ToTorchFormatTensor, read_mask
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True, **kwargs):
        ...
        if init_weights:
            self.init_weights()
        self.device_ = kwargs.get('device', None)
        self.half_ = kwargs.get('half_flag', None)
        self.w, self.h = 432, 240
        self.ref_stride = 10  # 参照帧间隔，STTN默认为10
        self.neighbor_stride = 15  # 参照的领域帧长度 [x-5:x+5+1]，STTN默认为5, 8g显存为13
        self.bboxs = []
    ...
    def get_ref_index(self, f, neighbor_ids, length, ref_length=10, num_ref=-1):
        '''
        DSTT中计算参照帧下标的算法
        :param neighbor_ids: 所有领域帧的index
        :param length:  视频长度
        :param ref_length: 参照帧步长
        :return: 所有参照帧的index
        '''
        ref_index = []
        if num_ref == -1:
